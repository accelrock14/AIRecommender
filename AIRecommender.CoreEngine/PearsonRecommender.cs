...
/*if (otherData.Length < baseData.Length)
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                sumXSq += baseData[i] * baseData[i];
                sumYSq += otherData[i] * otherData[i];
            }

            double cov = sumXY / n - (sumX * sumY) / (n * n);
            double sigmaX = Math.Sqrt(sumXSq / n - (sumX * sumX) / (n * n));
            double sigmaY = Math.Sqrt(sumYSq / n - (sumY * sumY) / (n * n));
...
{
    otherData.AddRange(new int[baseData.Count() - otherData.Count()]);
}*/
